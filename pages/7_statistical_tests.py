...
# Assume all non-numeric columns (except Timestamp) are categorical
categorical_cols = [col for col in data.columns if col not in numeric_cols and col.lower() != 'timestamp']
...
if selected_test == "Independent T-Test":
    ...
    if not numeric_cols or not categorical_cols:
        ...
        cat_var = st.selectbox("Select Grouping Variable (must have exactly 2 groups)", categorical_cols)
        ...
        if len(groups) != 2:
            ...
            if p_val < 0.05:
                result_str += "Result: Statistically significant difference between the groups."
            else:
...
# ----------------- Chi-Square Test ----------------- #
    ...
    if len(categorical_cols) < 2:
        ...
        result_str = (
            ...
            f"Chi2 = {chi2:.3f}, p-value = {p_val:.3f}, Degrees of Freedom = {dof}\n\n"
        )
        if p_val < 0.05:
            result_str += "Result: Statistically significant association exists between the variables."
        else:
...
# ----------------- One-Way ANOVA ----------------- #
    ...
    if not numeric_cols or not categorical_cols:
        ...
        num_var = st.selectbox("Select Numeric Variable", numeric_cols)
        cat_var = st.selectbox("Select Grouping Variable (categorical)", categorical_cols)
        groups = data[cat_var].dropna().unique()
        if len(groups) < 2:
